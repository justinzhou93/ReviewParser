...
function* run(link) {
    ...
    nextExists = yield nightmare.visible(`.${site.next}`);

    let dt = {good: {}, bad: {}};

    while (nextExists) {
        let obj = yield nightmare
            .evaluate((site, data) => {
    ...
    Object.keys(dt).forEach(goodbad => {
        Object.values(dt[goodbad]).forEach(word => {
            // stopwords
